...
describe("Stage 15 deployment admission boundary", () => {
  it("checks capacity and advances to PreparingArtifacts in the same transaction", async () => {
    let updateArgs: Prisma.AppGroupDeploymentUpdateArgs | undefined;
    const update = vi.fn((args: Prisma.AppGroupDeploymentUpdateArgs) => {
      updateArgs = args;
    ...
    const result = await service.admitAndAdvance(
      deployment,
      snapshot,
      "capacity admitted",
    );
    ...
    expect(updateArgs?.where).toEqual({ id: deployment.id });
    expect(updateArgs?.data.phase).toBe(DeploymentPhase.PreparingArtifacts);
    expect(updateArgs?.data.status).toBe(DeploymentStatus.Deploying);
