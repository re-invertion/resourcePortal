...
type AsyncVolumesController = {
  createVolume: (tenantId: string, dto: unknown, user: AuthenticatedUser) => Promise<unknown>;
};
type AsyncDomainsController = {
  validateDomain: (tenantId: string, domainId: string, user: AuthenticatedUser) => Promise<unknown>;
};

describe("Stage 16 async mutation endpoints", () => {
  it("queues Volume create instead of provisioning in the request", async () => {
    ...
    await controller.createVolume("22222222-2222-4222-8222-222222222222", { name: "data", sizeBytes: 1024 }, actor);

    expect(enqueue).toHaveBeenCalledWith(expect.objectContaining({ type: "VOLUME_CREATE", resourceType: "Volume", actor }));
    expect(createVolume).not.toHaveBeenCalled();
  });

  it("queues Domain verification instead of validating in the request", async () => {
    ...
    expect(enqueue).toHaveBeenCalledWith(expect.objectContaining({ type: "DOMAIN_VERIFY", resourceType: "Domain", actor }));
