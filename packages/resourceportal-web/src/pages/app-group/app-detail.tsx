...
export function ApplicationDetail({ tenantId, appGroupId, appId, section = "overview" }: { tenantId: string; appGroupId: string; appId: string; section?: ApplicationSection }) {
  ...
  async function runtimeAction(action: "start" | "stop" | "restart") {
    setWorking(action); setActionError(undefined);
    try { await apiRequest(`${appRoot}/runtime/${action}`, { method: "POST" }); await appsQuery.reload(); }
    catch (cause) { setActionError(cause instanceof Error ? cause.message : `Application ${action} failed.`); }
  ...
  return <section className="space-y-5">
    ...
    {actionError?<Callout tone="danger" title="Runtime action failed">{actionError}</Callout>:null}
