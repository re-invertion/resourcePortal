...
## Public WAN / NAT prerequisite

The public ResourcePortal and ZITADEL hostnames must resolve to the public ingress IPv4 address. When the ingress node itself uses a private RFC1918 address behind an edge router or ONT, that device must forward **TCP 80** and **TCP 443** from the public WAN address to the ResourcePortal ingress node on the same ports. The production installer configures the host firewall and Docker Swarm ingress, but it does not configure an external router, ONT, ISP NAT or CGNAT.

Before the `ingress` stage can obtain ACME certificates, verify that Internet-originated connections to both public ports actually reach the selected ingress node. Do not expose the private Swarm, NFS or enrollment ports to the public Internet. If the router WAN interface does not own the expected public IPv4 address because the connection is behind CGNAT or another upstream NAT, obtain a public address or configure the upstream forwarding before continuing.
...
During ZITADEL bootstrap, Resource Portal provisions separate OAuth clients for the Web Console and CLI. The CLI client is a public/native client with Device Authorization and JWT access tokens; it has no client secret. Only its non-secret client ID is persisted as `RP_CFG_OIDC_CLI_CLIENT_ID` and rendered to the API as `OIDC_CLI_CLIENT_ID`. Upgrade/repair final-deploy recovery idempotently creates or discovers this client for installations that predate browser CLI login, using the existing protected ZITADEL management/bootstrap credential without re-running first-admin bootstrap.
