...
not_contains "$enrollment_source" '--mount type=bind,src=/var/run/docker.sock' 'enrollment listener has no Docker socket'
not_contains "$enrollment_source" '--user 0:0' 'enrollment listener does not run as forced root'
not_contains "$enrollment_source" 'INSTALLER_SWARM_WORKER_TOKEN_FILE' 'enrollment listener has no worker join-token secret'
not_contains "$enrollment_source" 'INSTALLER_SWARM_MANAGER_TOKEN_FILE' 'enrollment listener has no manager join-token secret'
contains "$enrollment_source" "--constraint 'node.labels.rp.node.control-plane==true'" 'enrollment listener schedules by v0.2 control-plane role'
not_contains "$enrollment_source" "--constraint 'node.labels.resourceportal.control-plane==true'" 'enrollment listener no longer schedules by legacy control-plane role'
contains "$enrollment_source" '/installer/enrollment/redeem/status' 'client polls worker-backed redemption operation'
contains "$enrollment_source" 'openssl genpkey -algorithm RSA' 'client generates an ephemeral enrollment RSA key'
contains "$enrollment_source" 'openssl pkeyutl -decrypt' 'client decrypts the worker-encrypted join token locally'
...
contains "$issue_function" "--constraint 'node.labels.rp.node.storage==true'" 'enrollment issuer requires v0.2 storage role'
contains "$issue_function" "--constraint 'node.labels.resourceportal.storage.authoritative==true'" 'enrollment issuer retains authoritative storage capability guard'
