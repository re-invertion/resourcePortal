...
contains_file "$repo_root/.github/workflows/release.yml" "git log -1 --pretty=%B | grep -Fq '[fastfix]'" 'Release detects marked fastfix commits from git history'
contains_file "$repo_root/.github/workflows/release.yml" 'npm run test:fastfix:mcp-oauth' 'Release uses targeted validation for marked fastfix releases'
contains_file "$repo_root/.github/workflows/release.yml" "if: steps.release-mode.outputs.fastfix != 'true'" 'Release keeps full installer validation for normal releases'
